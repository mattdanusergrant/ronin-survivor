...
- **Meta-progression:** Ryo currency earned per run, spent on permanent upgrades, persisted in `localStorage` (`ronin-meta-v1`); a walkable Dojo hub (Spirit Forge + The Road) with training dummies and a lone guardian archer; a gated spiral world with boss seals, campfire checkpoints, and return-road enemy lanes; a sprite-art pipeline and a custom-map editor (`mapbuilder.html`). Mini-games (Samurai Soccer, Samurai Volleyball) are now an Options-menu easter egg rather than Dojo stations. See `design-docs/TODO.md` for parked redesigns (special buildings, enemy flow).
